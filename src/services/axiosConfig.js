...
spaceaClient.resetApiToken = () => {
  ...
  if (token) {
    spaceaClient.defaults.headers.common['Authorization'] = `Bearer ${token}`;
  }
  else {
    spaceaClient.defaults.headers.common['Authorization'] = undefined;
  }
};


const initInterceptors = (appStore, history) => {
  spaceaClient.interceptors.response.use(
    res => {
      return res.data;
    }, async err => {
            ...
            if (headers.hasOwnProperty('token-expired')) {
              ...
              if (!isRefreshing) {
                ...
                try {
                  ...
                  spaceaClient.resetApiToken && spaceaClient.resetApiToken();
                }
                finally {
                  isRefreshing = false;
                }
